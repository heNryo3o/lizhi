...
class NewsCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $list = NewsCategory::remember(10080)->orderBy($order_column, $order_type)->paginate($request->limit);
    ...
    public function create(CategoryRequest $request)
    {
        ...
        if($request->parent_id){
            ...
            $parent = NewsCategory::where('id',$data['parent_id'])->get('level')->first();
        ...
        $count = NewsCategory::where(['slug'=>$data['slug']])->count();
    ...
    public function edit(CategoryRequest $request)
    {
        ...
        $count = NewsCategory::where(['slug'=>$data['slug']])->where('id','!=',$request->id)->count();

        if($count > 0){
            return $this->failed('slug重复,请修改名称后重新提交');
        }

        NewsCategory::find($request->id)->update($data);
    ...
    public function changeStatus(ChangeStatusRequest $request)
    {
        ...
        NewsCategory::find($request->id)->update(['status'=>$request->status]);
        ...
        foreach ($parent as $k => $v){
            ...
            if($child){
                ...
                foreach ($child->toArray() as $vk => $vv){
                    ...
                    $children = NewsCategory::where(['parent_id'=>$vv['id']])->remember(10080)->get(['id','name']);
    ...
    public function subOptions(Request $request)
    {
        ...
        $parent = NewsCategory::where(['level'=>2,'parent_id'=>$request->is_online])->remember(10080)->get(['id','name'])->toArray();
